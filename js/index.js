...
function egg1OnFloor() {
  ...
  if (Math.round((egg1Bottom + egg1Top) / 2) >= floorTop) {
    brokenEgg1.style.display = "block";
    egg1Top = 40;
    egg1.style.top = egg1Top + "px";
    lifeCount--;
    life.innerHTML = lifeCount;
...
function egg1OnBasket() {
  var egg1X = Math.round(
    (egg1.getBoundingClientRect().left + egg1.getBoundingClientRect().right) / 2
  );
  ...
  if (egg1X >= basketLeft && egg1X <= basketRight) {
    if (
      basketTop == egg1Bottom ||
      (egg1Bottom >= basketTop && egg1Bottom <= basketTop + 50)
    ) {
      egg1Top = 40;
      egg1.style.top = egg1Top + "px";
      cout++;
...
function egg2OnBasket() {
  var egg2X = Math.round(
    (egg2.getBoundingClientRect().left + egg2.getBoundingClientRect().right) / 2
  );
  ...
  if (egg2X >= basketLeft && egg2X <= basketRight) {
    if (
      basketTop == egg2Bottom ||
      (egg2Bottom >= basketTop && egg2Bottom <= basketTop + 50)
    ) {
      egg2Top = 40;
      egg2.style.top = egg2Top + "px";
      cout++;
...
function egg3OnBasket() {
  var egg3X = Math.round(
    (egg3.getBoundingClientRect().left + egg3.getBoundingClientRect().right) / 2
  );
  ...
  if (egg3X >= basketLeft && egg3X <= basketRight) {
    if (
      basketTop == egg3Bottom ||
      (egg3Bottom >= basketTop && egg3Bottom <= basketTop + 50)
    ) {
      egg3Top = 40;
      egg3.style.top = egg3Top + "px";
      cout++;
